...
class Detector(QWidget):
    def __init__(self):
        ...
        self.resize(1600, 925)
        self.setWindowTitle("4C Detector")
        self.setWindowIcon(QIcon('ee.png'))
        ...
        self.le.setGeometry(QRect(100, 25, 1200, 30))
        font = QtGui.QFont()
    ...
    def locate(self):
        ...
        # 传输图像bytes
        url = 'http://{}:{}/detect'.format(ip_addr, port)
        im_encoded = cv2.imencode('.jpg', self.image)[1]
        im_bytes = im_encoded.tostring()
        r = requests.post(url, files={'image': im_bytes})
        if r.status_code == 200:
            self.loc = np.float32(r.json().get('loc')).reshape(1,-1,6)
            self.res = np.float32(r.json().get('res')).reshape(1,-1,6)
            print('detect {} objs'.format(self.loc.shape[1]))
    ...
    def inspect(self):
        self.draw_boxes(self.res, classes2, colors2)
